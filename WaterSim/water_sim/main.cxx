...
void onKeyboardCb(unsigned char key, int x, int y)
{
   ...
   else if (key == '0') MACGrid::theRenderMode = MACGrid::SPHERES;
   else if (key == '1') MACGrid::theRenderMode = MACGrid::BILLBOARD;
   else if (key == 'v') MACGrid::theDisplayVel = !MACGrid::theDisplayVel;
   else if (key == 'f') MACGrid::theDisplayFluReg = !MACGrid::theDisplayFluReg;
   else if (key == 'm') MACGrid::theDisplayMarkMesh = !MACGrid::theDisplayMarkMesh;
   else if (key == 'l') MACGrid::theDisplayLevSet = !MACGrid::theDisplayLevSet;
   else if (key == 's') MACGrid::theDisplaySol = !MACGrid::theDisplaySol;
   else if (key == 'r') theWaterSim.setRecording(!theWaterSim.isRecording());
   else if (key == '>') isRunning = true;
   else if (key == '=') isRunning = false;
   else if (key == '<') theWaterSim.reset();
...
void onMenuCb(int value)
{
   ...
   {
   case -1: exit(0);
   case -6: theWaterSim.reset(); break;
...
void drawOverlay()
{
  ...
  glPushAttrib(GL_LIGHTING_BIT);
     ...
     sprintf(info, "Framerate: %3.1f  %s", 
         ...
         theWaterSim.isRecording()? "Recording..." : "");
...
int main(int argc, char **argv)
{
    ...
    glutCreateWindow("Water Simulation - CIS563");
	...
	glutAddMenuEntry("Toggle Fluid/Air Regions\t'f'", 'f');
	glutAddMenuEntry("Toggle Level Set\t'l'", 'l');
	glutAddMenuEntry("Toggle Solids\t's'", 's');
